...
class AdvancedMalwareProtectionParcel(_ParcelBase):
    ...
    file_analysis_cloud_server: Global[Optional[FileAnalysisServer]] = Field(
        default=None, validation_alias=AliasPath("data", "fileAnalysisCloudServer")
    )
    file_analysis_file_types: Global[List[FileAnalysisFileTypes]] = Field(
        default=[], validation_alias=AliasPath("data", "fileAnalysisFileTypes")
    )
    file_analysis_alert: Global[Optional[FileAnalysisAlert]] = Field(
