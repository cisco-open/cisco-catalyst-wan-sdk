...
class DeviceAccessIPv4Parcel(_ParcelBase):
    ...
    def create_match_entries(
        ...
        destination_prefix: Optional[Union[RefIdItem, List[str], List[IPv4Interface], Variable]] = None,
        source_prefix: Optional[Union[RefIdItem, List[str], List[IPv4Interface], Variable]] = None,
        source_ports: Optional[List[int]] = None,
    ) -> MatchEntries:
        def resolve_ip_prefix_list(
            prefix_list: Union[List[str], List[IPv4Interface], Variable]
        ) -> Union[Global[List[IPv4Interface]], Variable]:
            if isinstance(prefix_list, list):
                return Global[List[IPv4Interface]](value=prefix_list)  # type: ignore
            else:
                return prefix_list
