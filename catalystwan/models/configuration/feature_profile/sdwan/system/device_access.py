...
class Sequence(BaseModel):
    ...
    def match_destination_data_prefix_ip_list(self, destination_prefix: Union[List[str], List[IPv4Interface]]):
        self.match_entries.destination_data_prefix = DestinationIPPrefix(
            destination_ip_prefix_list=Global[List[IPv4Interface]](value=destination_prefix)  # type: ignore
        )

    def match_destination_data_prefix_ip_variable(self, destination_prefix: str):
        self.match_entries.destination_data_prefix = DestinationIPPrefix(
            destination_ip_prefix_list=as_variable(destination_prefix)
        )
    ...
    def match_source_data_prefix_ip_list(self, source_prefix: Union[List[str], List[IPv4Interface]]):
        self.match_entries.source_data_prefix = SourceIPPrefix(
            source_ip_prefix_list=Global[List[IPv4Interface]](value=source_prefix)  # type: ignore
        )

    def match_source_data_prefix_ip_variable(self, source_prefix: str):
        self.match_entries.source_data_prefix = SourceIPPrefix(source_ip_prefix_list=as_variable(source_prefix))
