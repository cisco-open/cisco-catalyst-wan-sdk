...
class MultilinkTemplateConverter:
    ...
    def parse_controller_tx_ex_list(self, tx_ex_values: Optional[List[dict]]) -> Optional[List[ControllerTxExList]]:
        if not tx_ex_values:
            return []
        ...
        for tx_ex_value in tx_ex_values:
            ...
            tx_ex_value = normalize_to_model_definition(tx_ex_value, ControllerTxExList.model_fields)
    ...
    def parse_nim_list(self, nim_values: Optional[List[dict]]) -> Optional[List[NimList]]:
        if not nim_values:
            return None
        nim_list = []
        for nim_value in nim_values:
            ...
            normalized_value = normalize_to_model_definition(flattened_value, NimList.model_fields)
            nim_list.append(
                NimList(  # type: ignore
    ...
    def get_authentication_type(self, flattened_values: dict) -> Optional[Global[AuthenticationType]]:
        ...
        if callin.value is False:
            return Global[AuthenticationType](value="bidirectional")
        elif callin.value == "callin":
            return Global[AuthenticationType](value="unidirectional")
        else:
            return None

    def get_method(self, flattened_values: dict) -> Optional[Global[Method]]:
        method: Optional[Global[bool]] = flattened_values.get("method")
        pap: Optional[Global[bool]] = flattened_values.get("pap")
        if method is None:
            return None
        if method.value == "pap":
            return Global[Method](value="PAP")
        elif method.value == "chap":
            if pap:
                return Global[Method](value="PAP and CHAP")
            else:
                return Global[Method](value="CHAP")
        return None
